...
module Jekyll
  module Tumblr
    ...
    def self.html_to_markdown(content)
      ...
      #For complete gist with ID 
      content.gsub!(/<script src="https:\/\/gist.github.com\/(?<id>[\d]+).js"/,'{% gist \k<id> %} <script src="https://gist.github.com/\k<id>.js"')

      # For gists with ID and file
      content.gsub!(/<script src="https:\/\/gist.github.com\/(?<id>[\d]+).js\?file=(?<file>[\.\w]+)"/,'{% gist \k<id> \k<file> %} <script src="https://gist.github.com/\k<id>.js?file=\k<file>"')
